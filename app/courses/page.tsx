...
export default function CoursesPage() {
  ...
  const fetchTeeTime = async (courseName: string, courseUrl: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/tee-times/${courseName}`, {
        method: 'POST',
        headers: {
      ...
      setErrors(prev => ({
        ...prev,
        [courseName]: error instanceof Error ? error.message : 'Failed to fetch tee times'
      }))
    } finally {
    ...
    <CoursesContainer>
      <Header>
        <HeaderContent>
          ...
          <BackButton onClick={() => router.push('/home')}>Back to Home</BackButton>
        </HeaderContent>
      </Header>
      <Content>
        {Object.entries(courses).map(([courseName, courseUrl]) => (
          <CourseCard key={courseName}>
            ...
            {teeTimes[courseName]?.data && (
              <TeeTimeInfo>
                <h3>Tee Time Data:</h3>
                <TeeTimeText>{JSON.stringify(teeTimes[courseName].data, null, 2)}</TeeTimeText>
              </TeeTimeInfo>
            )}
          </CourseCard>
        ))}
